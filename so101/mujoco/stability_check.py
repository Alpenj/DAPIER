...
jdcobot100_sim/mujoco/stability_check.py와 같은 도구다. SO101은 6축이라
관절별 오차를 따로 찍는 부분만 다르다.
...
def report(res: dict) -> None:
    ...
    if "tracking_error" in res:
        ...
        print(f"    최대 오차 = {res['tracking_error'].max():.5f} rad "
              f"({np.degrees(res['tracking_error'].max()):.3f} deg)")
