...
uses a contact-gated hold constraint; the shoe has no weld or attachment.
"""
...
def _shoe_finger_contacts(
    ...
    fixed: list[mujoco.MjContact] = []
    moving: list[mujoco.MjContact] = []
...
def _finger_contact_normal_dot(
    ...
    return _most_opposing_normal_dot(
        [shoe_outward(contact) for contact in fixed],
        [shoe_outward(contact) for contact in moving],
    )
...
def _classify_unintended_contact(
    ...
    *,
    ...
    if distance_m >= -MAX_CONTACT_PENETRATION_M or len(set(pair)) < 2:
        return None
    if right_lid_hold_active and pair == (BOX_LID_BODY_NAME, "right_gripper"):
        return None
    if not (
...
class BoxShoePhysicsDemo:
    ...
    def _close_left_gripper(
        ...
        action: np.ndarray,
        viewer: mujoco.viewer.Handle | None,
    ) -> tuple[np.ndarray, list[mujoco.MjContact], list[mujoco.MjContact]]:
        closed = action.copy()
        closed[5] = LEFT_GRIPPER_CLOSED_RAD
        self._move(action, closed, viewer=viewer, phase="left_gripper_close")
        self._hold(closed, viewer, phase="left_gripper_close")
        fixed, moving = _shoe_finger_contacts(self.model, self.data)
        return closed, fixed, moving
    ...
    def run(self, viewer: mujoco.viewer.Handle | None = None) -> DemoReport:
        ...
        closed_action, fixed_contacts, moving_contacts = self._close_left_gripper(
            left_contact_action,
            viewer,
        )
        contact_normal_dot = _finger_contact_normal_dot(
            self.model, fixed_contacts, moving_contacts
        )
        if (
            not fixed_contacts
            or not moving_contacts
            or contact_normal_dot is None
            or contact_normal_dot > MAX_OPPOSING_CONTACT_NORMAL_DOT
        ):
            return self._report(
                "left_physical_grasp_failed",
                len(right_contacts),
                len(fixed_contacts) + len(moving_contacts),
                shoe_initial,
                fixed_contact_count=len(fixed_contacts),
                moving_contact_count=len(moving_contacts),
                contact_normal_dot=contact_normal_dot,
            )
        self._start_grasp_tracking()
        self._emit(
            "left_physical_grasp",
            fixed_contacts=len(fixed_contacts),
            moving_contacts=len(moving_contacts),
        ...
        fixed_contacts, moving_contacts = _shoe_finger_contacts(
            self.model, self.data
        )
        return self._report(
            "completed",
