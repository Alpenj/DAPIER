...
class MissionCoreTest(unittest.TestCase):
    ...
    def test_navigation_waits_for_healthy_transport_hold_ack(self) -> None:
        ...
        missing_ack_evidence = MissionController()
        request_hold(missing_ack_evidence)
        rejected = missing_ack_evidence.dispatch(
            self.event(MissionEventType.TRANSPORT_HOLD_RESULT)
        )
        self.assertEqual(rejected.phase, MissionPhase.SAFE_STOP_REQUESTED)
        self.assertNotIn(MissionCommand.NAVIGATE_TO_A, rejected.commands)
        ...
        missing_return_evidence = MissionController()
        request_hold(missing_return_evidence)
        missing_return_evidence.dispatch(
            self.event(
                MissionEventType.TRANSPORT_HOLD_RESULT,
                transport_hold_ok=True,
            )
        )
        rejected = missing_return_evidence.dispatch(
            self.event(
                MissionEventType.NAVIGATION_RESULT,
                location=MissionLocation.A,
            )
        ...
        self.assertEqual(rejected.phase, MissionPhase.SAFE_STOP_REQUESTED)
        self.assertNotIn(MissionCommand.PLACE_SHOE, rejected.commands)
