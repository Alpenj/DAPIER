...
from box_shoe_physics_demo import (
    MAX_OPPOSING_CONTACT_NORMAL_DOT,
    RIGHT_WING_GEOM_NAME,
    BoxShoePhysicsDemo,
...
class BoxShoeMissionTest(unittest.TestCase):
    ...
    def test_positive_evidence_defaults_fail_closed(self) -> None:
        event = BoxShoeEvent(
            kind=BoxShoeEventType.START,
            observation_seq=0,
            observation_age_ms=0.0,
        )
        self.assertFalse(event.success)
        self.assertFalse(event.base_stationary)
        self.assertEqual(event.pose_confidence, 0.0)
...
class BoxShoePhysicsDemoTest(unittest.TestCase):
    ...
    def test_collision_evidence_covers_structures_and_bounds_expected_contacts(
        self,
    ) -> None:
        for pair in (
    ...
    def test_shoe_box_penetration_is_accumulated_dynamic_evidence(self) -> None:
        ...
        for pair in (
            ...
            with self.subTest(intended_pair=pair):
                self.assertEqual(
                    _classify_unintended_contact(
                        pair,
                        distance_m=-0.010,
    ...
    def test_invalid_physical_contacts_do_not_verify_a_grasp(self) -> None:
        ...
        self.assertEqual(report.completed_phase, "left_opposing_contact_failed")
        ...
        self.assertGreater(report.left_moving_finger_contact_count, 0)
        self.assertTrue(report.bilateral_finger_contact_verified)
        self.assertFalse(report.opposing_finger_contact_verified)
        normal_dot = report.finger_contact_normal_dot
        self.assertIsNotNone(normal_dot)
        assert normal_dot is not None
        self.assertGreater(normal_dot, MAX_OPPOSING_CONTACT_NORMAL_DOT)
        ...
        self.assertIn(
            ...
            report.unintended_contact_phases,
        )
        wing_geom = demo.model.geom(RIGHT_WING_GEOM_NAME)
        self.assertNotEqual(int(wing_geom.contype[0]), 0)
        self.assertNotEqual(int(wing_geom.conaffinity[0]), 0)
