...
MOUNT_Y_M = 0.080
MOUNT_Z_M = 0.1095
CONCAVE_GRIPPER_MESHES = (
    "wrist_roll_follower_so101_v1.stl",
    "moving_jaw_so101_v1.stl",
)


def _convex_gripper_parts(
    mesh_source: Path, mesh_target: Path
) -> dict[str, list[str]]:
    if not all((mesh_source / name).is_file() for name in CONCAVE_GRIPPER_MESHES):
        return {}
    try:
        import coacd
        import trimesh
        ...
        raise RuntimeError(
            "RoboTwin gripper build needs `python -m pip install coacd`"
        ) from error

    coacd.set_log_level("warn")
    result = {}
    for name in CONCAVE_GRIPPER_MESHES:
        source = mesh_source / name
        for stale in mesh_target.glob(f"{source.stem}.collision.*"):
            stale.unlink()
        ...
        if len(hulls) < 2:
            raise RuntimeError(f"CoACD did not split concave gripper mesh: {source}")
        parts = []
        for index, (vertices, faces) in enumerate(hulls):
            part = f"{source.stem}.collision.{index:02d}.stl"
            trimesh.Trimesh(vertices, faces, process=False).export(mesh_target / part)
            parts.append(part)
        result[name] = parts
    return result
...
def _prefixed_arm(
    ...
    mesh_source: Path,
    mesh_target: Path,
    collision_parts: dict[str, list[str]],
) -> list[ET.Element]:
    ...
    for original in source:
        ...
        for collision in list(item.findall("collision")):
            ...
            name = Path(mesh.get("filename", "")).name if mesh is not None else ""
            if name not in collision_parts:
                continue
            insert_at = list(item).index(collision)
            item.remove(collision)
            for part in collision_parts[name]:
                split = deepcopy(collision)
                split.find("./geometry/mesh").set("filename", f"../meshes/{part}")
                item.insert(insert_at, split)
                insert_at += 1
...
def build(source_urdf: Path, mesh_source: Path, output: Path) -> Path:
    ...
    collision_parts = _convex_gripper_parts(mesh_source, mesh_target)
    ...
    for side, y in (("left", MOUNT_Y_M), ("right", -MOUNT_Y_M)):
        for element in _prefixed_arm(
            source, side, mesh_source, mesh_target, collision_parts
        ):
        ...
        # Reuse the calibrated MuJoCo jaw midpoint in the official URDF frame.
        robot.append(ET.Element("link", {"name": f"{side}_grasp_link"}))
        robot.append(
            _fixed_joint(
                f"{side}_grasp_joint",
...
def validate_with_sapien(urdf: Path) -> None:
    ...
    for side in ("left", "right"):
        moving_jaw = robot.find_link_by_name(f"{side}_moving_jaw_so101_v1_link")
        if moving_jaw is not None and len(moving_jaw.collision_shapes) < 2:
            raise RuntimeError(f"{side} concave moving jaw was not decomposed")
    print(f"PASS: {len(active)} joints, {len(links)} links, {urdf}")


def validate_with_sapien_subprocess(urdf: Path) -> None:
    # CoACD and SAPIEN load incompatible native runtimes in one process.
    subprocess.run(
        [
            sys.executable,
            "-c",
            "from pathlib import Path; from build_dual_so101 import "
            "validate_with_sapien; validate_with_sapien(Path(__import__('sys').argv[1]))",
            str(urdf),
        ],
