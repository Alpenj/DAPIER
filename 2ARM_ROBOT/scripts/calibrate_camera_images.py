...
def estimate(paths):
    ...
    return {"schema_version": 1, "kind": "ChArUco pinhole intrinsics candidate",
        ...
        "board_definition": DEFINITION, "quality_warnings": warnings,
        ...
        "limitations": ["One physical camera, fixed focus/resolution/crop/rotation must be confirmed separately.",
            "Print dimensions and flatness are not verified by an image fit.",
...
def self_test():
    ...
    with tempfile.TemporaryDirectory(prefix="charuco-offline-check-") as directory:
        ...
        args = SimpleNamespace(images=root, output=Path(directory) / "candidate",
                               camera="left_wrist", capture_notes="SYNTHETIC: fixed optics, raw, no crop/rotation")
        assert run(args) == 0
    ...
    print("PASS: projected board images -> detected IDs -> intrinsics; invalid input rejection; no hardware")
...
if __name__ == "__main__":
    ...
    if args.self_test:
        self_test()
    elif any(getattr(args, k) is None for k in ("images", "camera", "capture_notes", "output")):
        parser.error("images, camera, capture-notes and new output are required")
    else:
