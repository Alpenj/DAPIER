...
AI는 명령어 초안을 정리하고, ROS 2 로그와 오류 메시지를 읽고, 반복되는
검증 작업을 작은 스크립트로 나누는 데 도움을 줬습니다. 배선·전원·모터
움직임·캘리브레이션 결과·episode 성공 여부는 사람이 직접 확인합니다.
따라서 이 문서는 자동으로 완주된 결과 보고서가 아니라, 다음 실험에서
사람이 직접 이어서 실행할 체크리스트입니다.
